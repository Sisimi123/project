...
<template>
    <div>
        <Modal 
            ...
            <Form :label-width="100" :model="dataInfo" ref="dataInfo" :style="'overflow-x:hidden;overflow-y:auto;max-height:'+height+'px'">
                <div style="padding:5px">
                    ...
                    <Row>
                        <Col>
                            <img :src="diagram" :style="{'width':width-50+'px'}" onerror="this.src=''"/>
                        </Col>
                    </Row>
                    <Row v-show="existsProcess && taskList.length>0">
                        <Col>
                        ...
                        <Col>
                            <table style="width:100%;margin-top:10px;" id="operation">
                                ...
                                <tr v-for="(item,index) in nextTasks" :key="index">
                                    <td><input :type="item.selectType" name="transition" :value="item.value" :taskType="item.taskType" :checked="item.checked" @click="selectNextTask(item)"></td>
                                    <td>{{item.name}}</td>
                                    <td>
                                ...
                                <!-- <tr>
                                    ...
                                    <td colspan="3" style="text-align:left"> 
                                        <DatePicker type="datetime" v-model="dataInfo.endtime" placement="top-end" :clearable="false" :editable="false" confirm placeholder="请选择办理时间" style="width: 200px"></DatePicker>
                                    </td>
        ...
        <userselectModal v-model="userselect.show" :selectIds="userselect.selectIds" :deptIds="userselect.deptIds" :roleIds="userselect.roleIds"  @on-select-change="userSelectChange"></userselectModal>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        eachflowElement(handler){
            ...
            var flowEls=operation.querySelectorAll('[name=transition]');
    ...
    watch:{
        value(val){
            if(val){
                this.dataInfo={message:"",endtime:new Date()};
                this.eachflowElement(function(item, index){
                    item.checked=false;
                });
                this.params={
