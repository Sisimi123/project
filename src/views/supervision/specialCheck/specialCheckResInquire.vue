<template>
    <div>
        ...
        <entryModal :isSpecial='true' :isView='true' moduleIdName='specialCheckEntry' :taskid='taskid' :info='info' :width="1200" v-model="status.entryModal" @visibleChange="statusChange" :mask-closable="maskclosable" :styles="{top: '20px','z-index':1000}"></entryModal>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        handleExport () {
            // 导出
            window.open(this.$basePath + '/supervision/examine/exportExcel?model=' + encodeURI(JSON.stringify(this.query)) + '&exportType=2');
        },
        entryWin (row) {
            var _self = this;
