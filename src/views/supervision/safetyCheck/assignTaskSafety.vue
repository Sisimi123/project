...
<template>
  <div>
    <data-table
      ref="test"
      :url="'/supervision/safetyCheck/getSafetyAssignPage'"
      :params="query"
      :columns="columns"
      :autoInitTable="false"
          ...
          <Row>
            ...
            <Col span="4">
              <Form-item label="主体名称:" :label-width="110">
                <Input v-model="query.wheres.legalEntityName" placeholder="请输入..."></Input>
              </Form-item>
            </Col>
            <Col span="4">
...
export default {
  ...
  methods: {
    ...
    getCrewList(value) {
      ...
      this.$http
        .post('/supervision/safetyCheck/safetyAssign', obj)
        // 返回值
        .then(res => {
          if (res.success) {
    ...
    //导出
    exportWord(){
       window.open(this.$basePath + '/supervision/examine/exportExcel?model=' + encodeURI(JSON.stringify(this.query)) + '&exportType=7'); 
    },
  },
  computed: {
      checkedObjList () {
