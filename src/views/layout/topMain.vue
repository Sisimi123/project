...
<template>
  <div class="main topMain" :class="{'main-hide-text': shrink}" style="background:white">
    <div class="main-header" style="z-index:22;background: #2d8cf0; position: relative;">
      ...
      <span class="systemTitle">{{title}}</span>
      ...
      <div class="header-avator-con" style="width:210px;">
        ...
        <Dropdown style="margin-right:5px;" @on-click="pathChange">
...
export default {
  ...
  methods: {
    ...
    getChild(v) {
      let arr = this.menuList.filter(key => {
        return key.code == v;
      });
      if (!arr[0].children || arr[0].children.length < 1) {
        this.$router.push({
          path: arr[0].component
        });
      } else {
        this.pageTagsList = arr[0].children;
        this.$router.push({
          path: arr[0].children[0].component
        });
        this.$refs.tagsPageOpene.currentPageName = arr[0].children[0].code;
      }
    },
    getGuide() {
      var _self = this;
