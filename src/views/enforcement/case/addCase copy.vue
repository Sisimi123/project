...
<template>
    <div class="content-wrap transactionRegist">
                    ...
                    <Row>
                        <Col span="23">
                            <FormItem label="涉嫌" prop="illegalAct">
                                <Input v-model="caseInfo.illegalAct" placeholder="请输入涉案案由">
                        ...
                        <Col span="16"  v-show="caseInfo.caseType=='陆上'">
                            <FormItem label="地点:"  prop="caseHappenAddress">
                                <Input :readonly="ifEdit" v-model="caseInfo.caseHappenAddress" placeholder="请输入地点">
                                    <Button slot="append"  icon="location"  @click="showAMapModal=true"></Button>
                                </Input> 
                            </FormItem>
                        </Col>
                    </Row>	
                    ...
                    <Row v-for="(item, index) in caseInfo.enforcers" :key="'enforcers_'+index">
                        ...
                        <Col span="2" >
                            <ButtonGroup>
                                ...
                                <Button @click="()=>{deleteLawMan(index)}"  type="error" style="margin-left:5px;">删除</Button>
                            </ButtonGroup>
                        </Col>
                    </Row>
                    ...
                    <Row>
                        <Col span="8">
                            ...
                            <FormItem prop="shipname" label="船舶名称:" >
                                <Input :readonly="ifEdit" v-model="caseInfo.shipinfo.shipname" placeholder="请选择渔船">
                    ...
                    <Row>
                        <Col span="8">
                            <FormItem prop="litigantname" label="姓名:">
                                <Input :readonly="ifEdit" ref="litigantsName"  v-model="caseInfo.litigants.name" placeholder="请输入姓名"   :disabled="disableUnit" ></Input>
                            </FormItem>
                        </Col>
                        <Col span="8">
                    ...
                    <Row>
                        <Col span="24" >
                            <FormItem label="案件编号:"  prop="enforceType">
                                <Input :readonly="ifEdit" search v-model="caseInfo.enforceType" placeholder="请输入案件文种"  style="width:200px;display: inline-table;">
                                    <span slot="prepend" @click="enforceTypeShow=true" style="cursor:pointer">▼</span>
                                    <span slot="append">罚字</span>
                                </Input>
                                <Input :readonly="ifEdit" v-model="caseInfo.enforceYear" placeholder="请输入案件年份" style="width:200px;display: inline-table;">
        ...
        <discretionaryModal v-model="discretionaryShow" @selectDiscretionaryPower="selectDiscretionaryPower"></discretionaryModal>
        ...
        <areaselectModal v-model="handedCaseInfo.show" deptTypes="70" deptflag="30" @on-select-change="handedCase"></areaselectModal>
    </div>
</template>
<script>
                        ...
                        buttons.push(h('Button', {
                            ...
                            on: {
                                click: function() {
                                    ...
                                    _self.openWs(row.id,row.dtype,_self.wenshou.edit,true)
                                }
                            }
    ...
    computed:{
        caseInfoRules(){
            ...
            var rules= {
                ...
                otherlawMan:[{validator:function(rule, value, callback){
                ...
                enforceType:[{validator:function(rule, value, callback){
                        var type= _self.caseInfo.enforceType;
                        var year= _self.caseInfo.enforceYear;
                        var num= _self.caseInfo.enforceNum;
                        if (status>"1"&&(isEmpty(type)||isEmpty(year)||isEmpty(num))){
                            return callback(new Error('请输入案件编号'));
                        }
                    callback();
                    },trigger: 'blur'}],
                caseContent:[{validator:function(rule, value, callback){
                ...
                fact:[{validator:function(rule, value, callback){
                ...
                punishContent:[{validator:function(rule, value, callback){
                        var s= _self.caseInfo.punishContent;
                        if (status>"1"&&isEmpty(s)) {
                            return callback(new Error('请输入对外公开的处罚决定书全文或摘要'));
                        }
                    callback();
                    },trigger: 'blur'}],
                executeresult:[{validator:function(rule, value, callback){
        ...
        transferCase(){
            ...
            _self.$Modal.confirm({
                ...
                onOk:function(){
                    ...
                    _self.$http({
                        method:'get',
                        url:_self.$basePath+"/enforces/case/transfer/"+_self.caseInfo.id,
                        ...
                        var temp=isNull(litigants.name)?litigants.unitName:litigants.name;
					    if(!isNull(shipinfo.shipname)){
                            if("1"==caseInfo.enforceMain)
                                temp+="("+shipinfo.shipname+")";
                            else
                                temp=shipinfo.shipname+"("+temp+")";
                        }
                        ...
                        window.location.href="/#/enforcement/caseIndex"
                    })
