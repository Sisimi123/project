...
<template>
  <div>
    <data-table
      ref="test"
      :url="'/statistics/leisurelyShip/page'"
      :params="query"
      :columns="columns"
      :autoInitTable="false"
    ...
    >
      ...
      <div slot="footer">
        ...
        <Button type="primary" v-show="!ifEdit" @click="submitData('info')">确定</Button>
      </div>
    </Modal>
...
export default {
  ...
  data() {
    return {
      infoRules: {
        ...
        owner:[
          { required: true, message: "船东不能为空！", trigger: "blur" },
        ],
        cjg:[
          { required: true, message: "船籍港不能为空！", trigger: "blur" },
        ],
        tel:[
  ...
  methods: {
    ...
    onSelectShip(row) {
      this.info = row;
      this.info.cjg = row.portregistry
    ...
    deleteList(row) {
      this.$Modal.confirm({
        ...
        onOk: () => {
          this.$http.delete("/statistics/leisurelyShip/" + row.id).then(res => {
            console.log(res);
            if (res.success) {
              this.initSearch();
    ...
    submitData(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          if (this.title == "新增") {
            this.$http.post("/statistics/leisurelyShip", this.info).then(res => {
              if (res.success) {
                this.initSearch();
            ...
            this.$http.put(`/statistics/leisurelyShip/${this.info.id}`, this.info).then(res => {
              if (res.success) {
                this.initSearch();
                this.showDetail = false;
